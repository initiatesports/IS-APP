...
var PERIOD_VOID = { "2026 5-6月": ["梁心朗"],
  // 7-8月暫停：馬仲然(c6)、蘇穎悠(c1)、梁正軒(c2/c5 因病暫停)。不報新一期 → 7-8月學費 $0／豁免
  // （保留喺名單/可登入/歷史；已繳行唔郁）。
...
var CUSTOM_FEE = {
  "2026 7-8月": {
    ...
    // 孔善盈：c1 轉 c7（特殊安排，只適用於孔善盈）。5-6月已繳$1040內含豁免2堂(6/1、6/13)$260,
    // 順延抵去 7-8月 → 7-8月實付 $910（已 bake 入 due,唔另加 EXTRA_CREDITS 免雙重抵）。
    "孔善盈": { weekly:1, due:910, paid:0,
      note:"c1轉c7（特殊安排，只適用於孔善盈）",
      adjNote:"c7 7-8月學費，已扣 5-6月豁免2堂(6/1、6/13)$260順延 → 實付$910" }
  }
};
 ...
 */
const CLASSES = {
  c1:{ dayZh:"星期一", wd:1, time:"5–6pm",  students:["余悅","蔡芷彤","羅梓晉","羅君信","羅君浩","示範學員","蘇穎悠","黃玥晴"] },
  ...
  c7:{ dayZh:"星期六", wd:6, time:"3–4pm",  students:["劉家頤","鄭宇喬","鍾皓惟","周莉晶","何芯蕾","孔善盈"] },
...
// ⚠️ 老闆規定(2026-07-06)：系統唔可以自動取消/停課任何課堂（包括公眾假期）——淨係老闆自己喺教練端取消(cancelledSet)先算數。
// 過去日期唔改：假期自動剔走「只」套用喺此日之前嘅堂（grandfather 舊帳/舊 grid），此日起假期一律當正常課堂。
var HOLIDAY_AUTOCANCEL_BEFORE = "2026-07-06";
...
function sessionsFor(cid){
  ...
  while(cur<=end){
    var s=iso(cur);
    // 假期自動剔走只套用喺 HOLIDAY_AUTOCANCEL_BEFORE 之前（過去唔郁）；之後假期＝正常課堂，淨係教練端手動取消(cancel)先剔。
    var autoHol = hol[s] && s < HOLIDAY_AUTOCANCEL_BEFORE;
    if(!autoHol && !cancel[s]) out.push(s);
...
var STUDENT_JOIN = { "梁心朗":"2026-07-01", "黃玥晴":"2026-07-01",
  "李灝宏":"2026-05-23",
  "孔善盈":"2026-07-04" };  // c1 轉 c7（7-8月起）；入班日設 c7 第一堂 7/4，保留 5-6月 c1 已繳記錄、免推斷 c7 過往堂  // 李灝宏 由 c7 轉 c6（7月正式）；入班日設真正開始上 c6 嗰日 5/23，保留佢 5–6月 c6 補堂、又免 2–5月假推斷
...
function apiMakeup(p){
  ...
  if(!authParent_(p.name,p.code)) return {ok:false,err:"登入碼不正確，無法操作"};
  // 只擋「過去假期」或教練手動取消日；未來假期＝正常課堂（老闆規定），可補堂。與 sessionsFor 同一 cutoff。
  if(holidaysSet()[date] && date < HOLIDAY_AUTOCANCEL_BEFORE) return {ok:false,err:"該日為假期／停課日（如 7/1），不能補堂，請另揀日期"};
...
function periodPaid_(nm, label){
  // 示範帳號（示範學員）：永遠當已繳，令每日健康檢查嘅寫入測試（請假/取消）唔會被學費閘攔住。
...
// nested：{ date: { "_all":{centre,room}, c1:{centre,room}, ... } }；班別空＝_all（該日全部班）
function venueMap_(){
  var m={};
...
function apiSetVenue(p){
  ...
  if(!centre && !room){ if(hit) sh.deleteRow(hit.row); return {ok:true, cleared:true}; }   // 兩個都空＝清除該日該班
  if(hit){ sh.getRange(hit.row,1,1,5).setValues([[date,cid,centre,room,nowStamp_()]]); }
  else { var nr=sh.getLastRow()+1; sh.getRange(nr,1).setNumberFormat("@"); sh.getRange(nr,1,1,5).setValues([[date,cid,centre,room,nowStamp_()]]); }
  return {ok:true};
}
...
function apiWeeklyText(p){
  ...
  var out=Object.keys(all).sort().map(function(d){
    ...
    var sess=(byDate[d]||[]).slice().sort(function(a,b){return a.h-b.h;});
    // 按場地分組：同場地嘅班合併一行時間；唔同場地各自一段
    var groups=[], gkey={};
    sess.forEach(function(s){
      var v=venueFor_(vmap,d,s.cid)||{centre:"",room:""}, k=(v.centre||"")+"|"+(v.room||"");
      if(gkey[k]==null){ gkey[k]=groups.length; groups.push({v:v, times:[]}); }
      groups[gkey[k]].times.push(s.time);
    });
    var body=groups.map(function(g){
      var v=g.v, times=g.times.join(" & ");
      var centreLine = v.centre ? (v.centre==="青衣體育館"?v.centre:("❗"+v.centre+"❗")) : "（地點待定）";
      var roomLine = v.room ? (v.room+"："+times) : times;
      return centreLine+"\n"+roomLine;
    }).join("\n");
    return head+"\n"+body;
  });
...
/* 一次性：把示範帳號 示範學員 現有「豁免」繳費列改返「未繳」（先自動備份）*/
function fixDemoUnexempt(){
  try{ backup(); }catch(e){}                          // 先備份，確保可還原
  var nm="示範學員", sh=feeSheet(), n=0;
  ...
  try{ SpreadsheetApp.getUi().alert("已把示範帳號 示範學員 嘅 "+n+" 筆「豁免」改返「未繳」。\n家長端用 1234 登入、揀對應月份就會見到收費功能區。"); }catch(e){}
  return n;
}
...
function ptRateSheet_(){
  ...
  if(!sh){ sh=SS().insertSheet("私訓費率"); sh.getRange(1,1,1,3).setValues([["學生","每堂費用","更新時間"]]); sh.setFrozenRows(1); sh.getRange("A:A").setNumberFormat("@"); }
  return sh;
}
function ptRate_(name){
  name=String(name||"").trim(); var sh=ptRateSheet_(); if(sh.getLastRow()<2) return 0;
  var v=sh.getRange(2,1,sh.getLastRow()-1,2).getValues(), rate=0;
  v.forEach(function(r){ if(String(r[0]||"").trim()===name) rate=Number(r[1])||0; });
  return rate;
}
function ptSetRate_(name, rate){
  name=String(name||"").trim(); rate=Math.max(0, Number(rate)||0);
  var sh=ptRateSheet_(), v=sh.getLastRow()>1?sh.getRange(2,1,sh.getLastRow()-1,1).getValues():[], found=0;
  for(var i=0;i<v.length;i++){ if(String(v[i][0]||"").trim()===name){ found=i+2; break; } }
  if(found){ sh.getRange(found,2,1,2).setValues([[rate, nowStamp_()]]); }
  else { var nr=sh.getLastRow()+1; sh.getRange(nr,1).setNumberFormat("@"); sh.getRange(nr,1,1,3).setValues([[name, rate, nowStamp_()]]); }
  return rate;
}
/* 某恆常班家庭(famName)本期已入帳嘅 PT 學費 = 該家庭旗下 PT 學員、已蓋「本期」章嘅堂數 × 費率。
   併入 genPeriodNet_ 嘅 adj → 每次重算都包含,唔會消失。Keith&Elaine 等 name≠family 亦對應返 family 帳戶。*/
...
function apiPtCoachLoad(p){
  if(String(p.coachPass)!==String(CONFIG.COACH_PASS)) return {ok:false,err:"密碼錯誤"};
  return {ok:true, cap:PT_CYCLE, today:todayIso(), curPeriod:curPeriodLabel_(),
...
function apiPtUnbill(p){
  ...
  var sh=ptSheet(), rows=ptRows_().filter(function(r){ return r.name===name && (label? r.billPeriod===label : !!r.billPeriod); });
...
/* 種私訓資料：示範帳號 示範學員（跟恆常班節奏，每週一堂、穩定上課，現處第 1 期）
   ＋ 鄧可澄(2026-06-12)、Keith & Elaine(2026-06-13) 實際記錄。
   全部 idempotent：已有同日記錄就跳過，可安全重複執行。 */
function seedPtData(){
  // (1) 示範帳號 示範學員：跟恆常班 c1（星期一）節奏，每週一堂，已連續上 7 堂（現 7/10，似快完一期嘅恆常學生）
  var demo="示範學員";
...
// ═══════ 寫入鏈自我測試：用示範帳號 示範學員 行真實「請假→讀寫驗證→取消→驗證清乾淨」round-trip ═══════
...
function writePathCheck_(){
  var P=[], nm="示範學員", cid="c1", today=todayIso();
...
/* ═══════════ 清理「補堂」表重複行 ═══════════
 * 背景：補堂表曾出現同一筆補堂被寫入多次（例如示範學員 c1→c3 同日 ×14），
...
function purgeStudentData_(nm){
  nm=String(nm||"").trim(); if(!nm) return {ok:false, err:"缺姓名"};
  var inRoster=CLASS_IDS.some(function(cid){ return CLASSES[cid].students.indexOf(nm)>=0; });
  if(inRoster) return {ok:false, err:"「"+nm+"」仍喺名冊，拒絕清除（請先由 CLASSES 移除）"};
  backupToDrive(); backup();
  var out={makeup:0, fee:0};
  var M=makeupSheet();
  if(M && M.getLastRow()>1){
    var mr=[]; makeupAll().forEach(function(m){ if(m.name===nm) mr.push(m.row); });
    mr.sort(function(a,b){return b-a;}).forEach(function(r){ M.deleteRow(r); }); out.makeup=mr.length;
  }
  var F=feeSheet();
  if(F && F.getLastRow()>1){
...
function seedDemo_(){
  ...
  // 註：示範帳號 示範學員 繳費不再設「豁免」（補堂示範用本期日子，periodPaid_ 本期及之前自動放行，唔會被閘）。
  var today=todayIso();
