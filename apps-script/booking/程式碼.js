...
// 🚫 提前暫停（2026-06-27 老闆要求）：無視 closeDate / forceOpen，報名一律截止、伺服器端拒收。
// 日後若要重開呢個 6-7月特別課程，改回 false 即可。
var FORCE_SUSPEND = true;

// 報名係咪已關閉:到期 且 老闆未強制重開 → 關閉。
// (專案時區已設 Asia/Hong_Kong,所以 new Date() 本身就係香港時間)
function isRegClosed_(cfg){
  if (FORCE_SUSPEND === true) return true;             // 強制提前暫停（蓋過 closeDate/forceOpen）
  cfg = cfg || readConfig();
